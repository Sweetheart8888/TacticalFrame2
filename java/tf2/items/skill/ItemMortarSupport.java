...
public class ItemMortarSupport extends ItemSkillBase
{
	private final int coolTime;
	public ItemMortarSupport(String name, int cooltime)
	{
		super(name);
		...
		if (x1 != null)
		{
			if (!worldIn.isRemote)
			{
				for (int i = 0; i < 3; i++)
                {
					...
					entity.setDamage(entity.getDamage() + 30.0F);
					entity.posX = x1.hitVec.x + ((worldIn.rand.nextDouble() * 10D) - 5D);
					entity.posZ = x1.hitVec.z + ((worldIn.rand.nextDouble() * 10D) - 5D);
					entity.posY = x1.hitVec.y + 60F - (i * 5F);
					entity.shoot(0D, -1D, 0D, 0.3F * i, 10.0F);
		...
		else
		{
			if (!worldIn.isRemote)
			{
				for (int i = 0; i < 3; i++)
                {
					EntityShell entity = new EntityShell(worldIn, playerIn);
					entity.setDamage(entity.getDamage() + 30.0F);
					entity.posX = playerPosX + vecX + ((worldIn.rand.nextDouble() * 10D) - 5D);
					entity.posZ = playerPosZ + vecZ + ((worldIn.rand.nextDouble() * 10D) - 5D);
					entity.posY = playerPosY + vecY + 60F - (i * 5F);
					entity.shoot(0D, -1D, 0D, 0.3F * i, 10.0F);
					worldIn.spawnEntity(entity);
                }
			}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
        ...
        {
        	tooltip.add(TextFormatting.BLUE + " " + I18n.translateToLocal("skill.mortarsupport"));
        }
        else
        {
        	tooltip.add(ChatFormatting.BLUE + " " + ("30") + " " + I18n.translateToLocal("tf.bombdamage"));
