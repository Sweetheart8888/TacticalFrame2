...
public class ItemCurePod extends ItemSkillBase
{
	private final int coolTime;
	public ItemCurePod(String name, int cooltime)
	{
		super(name);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
	{
		...
		List entity = worldIn.getEntitiesWithinAABB(EntityPlayer.class, playerIn.getEntityBoundingBox().grow(7.0D));
 		for (int i = 0; i < entity.size(); ++i)
 		{
 			EntityPlayer player = (EntityPlayer)entity.get(i);
 			if (!worldIn.isRemote)
 			{
				player.addPotionEffect(new PotionEffect(TFPotionPlus.DEBUFF_GUARD, 300, 0));
 			}
 		}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
        ...
        {
        	tooltip.add(TextFormatting.BLUE + " " + I18n.translateToLocal("skill.curepod"));
        }
        else
        {
        	tooltip.add(TextFormatting.BLUE + I18n.translateToLocal("debuff_guard") + " " + "(0:15)");
