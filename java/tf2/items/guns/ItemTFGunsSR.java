...
public class ItemTFGunsSR extends ItemTFGuns implements IHasModel
{
	...
	public ItemTFGunsSR(String name, int magazineSize, float bulletDamage, float levelUp, float bulletGrouping, SoundEvent gunSound, float gunRecoil, float gunRecoilRondom, int cooldown, int reloadTime)
	{
		super();
	...
	public void onPlayerStoppedUsing(ItemStack itemstack, World world, EntityLivingBase entity, int i)
	{
		if (!isReload(itemstack))
		{
			...
			if (entity != null && entity instanceof EntityPlayer)
			{
				...
				{
					boolean skill = this.chargeFlag(itemstack, entityplayer, i);

					fireBullet(itemstack, world, entityplayer, skill);
	...
	//弾の発射
	protected void fireBullet(ItemStack itemstack, World world, EntityPlayer entityplayer, boolean flag)
	{
		...
		boolean lflag = getFire(itemstack, world, entityplayer, flag);
	...
	public boolean chargeFlag(ItemStack itemstack, EntityPlayer entityplayer, int i)
	{
		boolean flag = false;
		...
		if (f > 1.0F)
		{
			for (int inv = 9; inv < 18; inv++)
			{
				...
				if (itemstack2 != null && itemstack2.getItem() == TFItems.SKILL_PRECISONSNIPER)
				{
					flag = true;
					break;
				}
	...
	//弾の設定
	public boolean getFire(ItemStack itemstack, World world, EntityPlayer entityplayer, boolean flag)
	{
		// 発射
		if (!world.isRemote)
		{
			...
			if (bullet != null)
			{
				...
				if (flag)
				{
					bullet.setDamage(bullet.getDamage() + (this.damage(itemstack) * 0.5F));
				}
				...
				//弾の弾速と集団性
				bullet.shoot(bullet.motionX, bullet.motionY, bullet.motionZ, 5.0F, this.group * b);

				world.spawnEntity(bullet);
			}
		}
		world.playSound(entityplayer, entityplayer.posX, entityplayer.posY, entityplayer.posZ, this.sound, SoundCategory.AMBIENT, 2.8F, 1.0F / (this.itemRand.nextFloat() * 0.4F + 0.8F));
	...
	{
		if (pItemstack != null && pItemstack.getItem() == TFItems.BOX_SNIPER)
		{
			return true;
		}
		return false;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
		{
			...
			tooltip.add(TextFormatting.GRAY + " " + I18n.translateToLocal("tf.bullettype") + " " + I18n.translateToLocal("tf.sniper"));
		}
		else
		{
