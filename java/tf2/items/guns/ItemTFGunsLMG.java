...
public class ItemTFGunsLMG extends ItemTFGuns implements IHasModel
{
	...
	public ItemTFGunsLMG(String name, int magazineSize, float bulletDamage, float levelUp, float bulletGrouping, SoundEvent gunSound, float gunRecoil, float gunRecoilRondom, int fireRate, int reloadTime)
	{
		super();
		...
		if (!world.isRemote)
		{
			...
			if (bullet != null)
			{
				...
				for (int inv = 9; inv < 18; inv++)
				{
					...
					if (itemstack2 != null && itemstack2.getItem() == TFItems.SKILL_FIREBARRAGE && this.getDamage(itemstack) % 4 == 0)
					{
						bullet.setDamage(bullet.getDamage() + this.damage(itemstack));
						break;
					}
				...
				//弾の弾速と集団性
				bullet.shoot(bullet.motionX, bullet.motionY, bullet.motionZ, 3.5F, group * b);
