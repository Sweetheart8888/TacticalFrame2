...
public class ItemTFGunsAR extends ItemTFGuns implements IHasModel
{
	...
	public ItemTFGunsAR(String name, int magazineSize, float bulletDamage, float levelUp, float bulletGrouping, SoundEvent gunSound, float gunRecoil, float gunRecoilRondom, int fireRate, int reloadTime)
	{
		super();
	...
	public boolean getFire(ItemStack itemstack, World world, EntityPlayer entityplayer)
	{
		if (!world.isRemote)
		{
			...
			for (int inv = 9; inv < 18; inv++)
			{
				...
				if (itemstack2 != null && itemstack2.getItem() == TFItems.SKILL_CORROSIONBULLET &&
			...
			{
				// 発射
				EntityBulletCorrosion bullet = null;

				bullet = new EntityBulletCorrosion(world, (EntityLivingBase) entityplayer);

				if (bullet != null)
				{
					//銃弾のダメージ
					bullet.setDamage(bullet.getDamage() + this.damage);
					float b = 1.0F;
					...
					//弾の弾速と集団性
					bullet.shoot(bullet.motionX, bullet.motionY, bullet.motionZ, 4.0F, group * b);
	...
	{
		if (pItemstack != null && pItemstack.getItem() == TFItems.BOX_RIFLE)
		{
			return true;
		}
		return false;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
		{
			...
			tooltip.add(TextFormatting.GRAY + " " + I18n.translateToLocal("tf.bullettype") + " " + I18n.translateToLocal("tf.rifle"));
		}
		else
		{
