...
public class ItemTFGunsGL extends ItemTFGuns implements IHasModel
{
	...
	public ItemTFGunsGL(String name, int magazineSize, float bulletDamage, float levelUp, double explodeSpread, float bulletGrouping, SoundEvent gunSound, float gunRecoil, float gunRecoilRondom, int cooldown, int reloadTime)
	{
		super();
	...
	public boolean getFire(ItemStack itemstack, World world, EntityPlayer entityplayer)
	{
		...
		if (!world.isRemote)
		{
			EntityGrenade bullet = null;

			bullet = new EntityGrenade(world, (EntityLivingBase) entityplayer);
			...
			if (bullet != null)
			{
				...
				bullet.setSpread(bullet.getSpread() + this.spread);
				for (int inv = 9; inv < 18; inv++)
				{
					ItemStack itemstack2 = entityplayer.inventory.getStackInSlot(inv);
					if (itemstack2 != null && itemstack2.getItem() == TFItems.SKILL_SPREADHOWITZER)
					{
						bullet.setSpread(bullet.getSpread() + this.spread);
						break;
					}
				...
				//弾の弾速と集団性
				bullet.shoot(bullet.motionX, bullet.motionY, bullet.motionZ, 3.0F, this.group * b);
		...
		//entityplayer.playSound(this.sound, 2.8F, 1.0F / (this.itemRand.nextFloat() * 0.4F + 0.8F));
	...
	{
		if (pItemstack != null && pItemstack.getItem() == TFItems.BOX_GRENADE)
		{
			return true;
		}
		return false;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
		...
		{
			tooltip.add(TextFormatting.GRAY + " " + (v) + " " + I18n.translateToLocal("tf.spread"));
			...
			tooltip.add(TextFormatting.GRAY + " " + I18n.translateToLocal("tf.bullettype") + " " + I18n.translateToLocal("tf.grenade"));
		}
		else
		{
