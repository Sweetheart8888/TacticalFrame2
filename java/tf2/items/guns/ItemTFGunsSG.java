...
public class ItemTFGunsSG extends ItemTFGuns implements IHasModel
{
	...
	public ItemTFGunsSG(String name, int magazineSize, float bulletDamage, float levelUp, int bulletAmount, float bulletGrouping, SoundEvent gunSound, float gunRecoil, float gunRecoilRondom, int cooldown, int reloadTime)
	{
		super();
		...
		if (!world.isRemote)
		{
			...
			for (int var11 = 0; var11 < e; var11++)
			{
				...
				if (bullet != null)
				{
					...
					//弾の初期弾速と集弾性の下限値
					bullet.setHeadingFromThrower(entityplayer, entityplayer.rotationPitch, entityplayer.rotationYaw, 0.0F, 2.0F, 1.0F);
					//弾の弾速と集団性
					bullet.shoot(bullet.motionX, bullet.motionY, bullet.motionZ, 2.0F, this.group * b);
	...
	{
		if (pItemstack != null && pItemstack.getItem() == TFItems.BOX_SHOT)
		{
			return true;
		}
		return false;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
		...
		tooltip.add(ChatFormatting.BLUE + I18n.translateToLocal(" ") + (i) + "x" + (k) + " " + I18n.translateToLocal("tf.bulletdamage"));
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
		{
			...
			tooltip.add(TextFormatting.GRAY + " " + I18n.translateToLocal("tf.bullettype") + " " + I18n.translateToLocal("tf.shotgun"));
		}
		else
		{
